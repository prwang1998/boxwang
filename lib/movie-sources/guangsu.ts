...
export class GuangsuSource extends MovieSource {
  readonly name = 'guangsu';
  readonly label = '光速资源';
  readonly icon = '⚡';
  readonly api = 'https://api.guangsuapi.com/api.php/provide/vod/';
  ...
  async healthCheck(): Promise<boolean> {
    try {
      const data = await this.cmsFetch({ ac: 'detail', wd: '测试', page: '1' });
      return data.code === 1;
    } catch {
      return false;
    }
