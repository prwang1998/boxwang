...
const SOURCE_OPTIONS = [
  { value: 'kuwo', label: '酷我', desc: '免费' },
  { value: 'netease', label: '网易云', desc: '需要VIP' },
  { value: 'musicbox', label: 'MusicBox', desc: '第三方' },
];

export default function MusicSearch({ onSearch, loading, currentSource = 'kuwo' }: MusicSearchProps) {
  const [keyword, setKeyword] = useState('');
  const [focused, setFocused] = useState(false);
  const { theme } = useTheme();

  const handleSearch = (source?: string) => {
    if (keyword.trim()) {
      onSearch(keyword.trim(), source || currentSource);
    }
  };
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {SOURCE_OPTIONS.map((opt) => (
          <button
            key={opt.value}
            onClick={() => handleSearch(opt.value)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 ${
              currentSource === opt.value
                ? 'bg-primary/15 text-primary border border-primary/20'
                : 'bg-white/[0.04] text-obsidian-100 border border-white/[0.06] hover:bg-white/[0.08]'
            }`}
          >
            {opt.label}
          </button>
        ))}
        <span className="text-[10px] text-obsidian-100/60">
          {SOURCE_OPTIONS.find(o => o.value === currentSource)?.desc}
        </span>
      </div>
